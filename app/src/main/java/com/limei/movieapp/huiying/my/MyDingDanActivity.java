...
public class MyDingDanActivity
    ...
    private Handler handler = new Handler(new Handler.Callback() {
        public boolean handleMessage(Message paramAnonymousMessage) {
            if (paramAnonymousMessage.arg1 == 0) {
                MyDingDanActivity.this.nulldata.setVisibility(View.GONE);
                manager = new LinearLayoutManager(MyDingDanActivity.this);
                adapter = new MyDingDanAdapter(MyDingDanActivity.this, MyDingDanActivity.this.myDingDanInfo);

                MyDingDanActivity.this.piaojiashow.setLayoutManager(MyDingDanActivity.this.manager);
                MyDingDanActivity.this.piaojiashow.setAdapter(MyDingDanActivity.this.adapter);
                return false;
            }
            if (paramAnonymousMessage.arg1 == 1) {
                //MyDingDanActivity.this.nulldata.setVisibility(View.VISIBLE);
                return false;
            }
            if (paramAnonymousMessage.arg1 == 2) {
                MyDingDanActivity.this.nulldata.setVisibility(View.VISIBLE);
                Toast.makeText(MyDingDanActivity.this, "暂无数据", Toast.LENGTH_SHORT).show();
            }
            return false;
        }
    });
    ...
    private void getData() {
        ...
        HttpOk.GetData(WebAdds.HTTPDINGDANJILU, localFormBody, new Callback() {
            ...
            public void onResponse(Call paramAnonymousCall, Response paramAnonymousResponse)
                ...
                Gson gson = new Gson();
                try {
                    myDingDanInfo = gson.fromJson(s, MyDingDanInfo.class);

                    if (MyDingDanActivity.this.myDingDanInfo.getCode().equals("000")) {
                        Message message = new Message();
                        message.arg1 = 0;
                        handler.sendMessage(message);

                    }
                    if (MyDingDanActivity.this.myDingDanInfo.getCode().equals("002")) {
                        Message message = new Message();
                        message.arg1 = 2;
                        handler.sendMessage(message);
                    }
...
/* Location:              G:\huiyingAPK\jd-gui-windows-1.4.0\classes-dex2jar.jar!\com\limei\movieapp\huiying\my\MyDingDanActivity.class
